// Regression test: cross-crate generic instantiation.
//
// Upstream generics instantiated by a downstream crate must be codegen'd in
// the instantiating crate (the defining crate never saw the instantiation),
// their closure classes must be defined locally, and classes emitted by
// several crates with complementary method sets must be merged by the
// java-linker rather than first-wins deduplicated. Failures here historically
// showed up as NoSuchMethodError / NoClassDefFoundError at runtime.
