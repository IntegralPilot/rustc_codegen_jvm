...
/// Parses a JVM method descriptor and returns the return type as a string.
pub fn parse_jvm_descriptor_return(descriptor: &str) -> Result<String, String> {
    ...
    let close_paren = descriptor
        .find(')')
        .ok_or_else(|| "Descriptor missing closing ')'".to_string())?;

    // 2. slice *after* the ')'
    let ret = descriptor
        .get(close_paren + 1..)
        .ok_or_else(|| "Descriptor missing return type".to_string())?;

    // 3. make sure we actually got something
    if ret.is_empty() {
        return Err("Descriptor missing return type".to_string());
    }
    ...
    match chars.next().unwrap() {
        'L' => {
            println!("Object type: {}", ret);
            // object type: find the terminating ';'
            let semicolon = ret
                .find(';')
                .ok_or_else(|| "Unterminated object type (missing `;`)".to_string())?;
            Ok(ret[..=semicolon].to_string())
        }
        '[' => {
            println!("Array type: {}", ret);
            // array type: everything from here to end
            Ok(ret.to_string())
        }
        prim => {
            println!("Primitive type: {}", prim);
            // primitive or void
            // 'V' is void, 'I' is int, 'Z' is boolean, etc.
            Ok(prim.to_string())
        }
    }
}