...
pub(super) fn build_stack_map_attributes(
    ...
    for target in target_offsets {
        ...
        previous_offset = Some(target);
        frames.push(StackFrame::FullFrame {
            frame_type: 255,
            offset_delta,
            locals: locals_for_stack_map(
                &state.locals,
                constant_pool,
                &mut verification_class_cache,
            )?,
            stack: stack_for_stack_map(&state.stack, constant_pool, &mut verification_class_cache)?,
        });
    }
